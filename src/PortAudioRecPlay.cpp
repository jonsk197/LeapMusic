...
int main(int argc, char* argv[])
{
	try
	{
		...
		AudioBuffer	objAudioBuffer((float) (SAMPLE_RATE * 60));
		...
		portaudio::DirectionSpecificStreamParameters inParamsRecord(sys.deviceByIndex(iInputDevice), 1, portaudio::FLOAT32, false, sys.deviceByIndex(iInputDevice).defaultLowInputLatency(), NULL);
		...
		portaudio::DirectionSpecificStreamParameters outParamsPlayback(sys.deviceByIndex(iOutputDevice), 1, portaudio::FLOAT32, false, sys.deviceByIndex(iOutputDevice).defaultLowOutputLatency(), NULL);
